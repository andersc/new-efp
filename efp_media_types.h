...
namespace PayloadType {
    ...
    constexpr uint8_t PRIVATE_DATA = 0x01;  // User-defined format
    ...
    constexpr uint8_t PCM_AUDIO    = 0x08;  // AES-3 framing
    constexpr uint8_t NDI          = 0x09;  // NDI format
    constexpr uint8_t JSON         = 0x0A;  // RFC 8259 JSON

    // Types requiring payloadCode (MSB set)
    constexpr uint8_t EFP_SIGNAL   = 0x80;  // EFP signaling (code: JSON/BINR)
    constexpr uint8_t DID_SDID     = 0x81;  // DID/SDID format (code: FOURCC)
...
namespace EmbeddedType {
    ...
    constexpr uint8_t PRIVATE_DATA = 0x01;  // Private embedded data
    constexpr uint8_t H222_PMT     = 0x02;  // PMT from H.222
    constexpr uint8_t MP4_FRAG_BOX = 0x03;  // MP4 fragment boxes (excluding payload)
    constexpr uint8_t LAST_MARKER  = 0x80;  // MSB set = last embedded section
}
